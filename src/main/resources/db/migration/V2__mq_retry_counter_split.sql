-- Migrate mq_message_log retry fields for existing databases.
...
SET @add_send_col_sql := IF(
    @send_col_exists = 0,
    'ALTER TABLE mq_message_log ADD COLUMN send_retry_count INT UNSIGNED NOT NULL DEFAULT 0 COMMENT ''Producer send retry count'' AFTER consume_status',
    'SELECT 1'
);
...
SET @add_consume_col_sql := IF(
    ...
    'ALTER TABLE mq_message_log ADD COLUMN consume_retry_count INT UNSIGNED NOT NULL DEFAULT 0 COMMENT ''Consumer retry count'' AFTER send_retry_count',
    'SELECT 1'
);
...
-- 3) backfill from legacy retry_count when it exists
